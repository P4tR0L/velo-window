...
- FR-008: User can create an account and log in via email+password or third-party identity provider. Priority: must-have
  > Socrates: No counter-argument; stands as written.
...
Login with email + password or third-party identity provider authentication. Flat user model — every authenticated user sees the same capabilities. No admin role, no role separation. Unauthenticated users cannot access the app beyond the login/sign-up screen.
...
- **No sport platform integrations** — no sync with external fitness ecosystems for MVP.
- **No custom weather prediction model** — rely entirely on external weather data; building proprietary forecasting is out of scope permanently.
...
None — all discovery phases completed and quality cross-check accepted with no gaps.
